...
#define CONFIG_ZTE_BATTERY_LEVEL 1
#define CONFIG_ZTE_CHARGE_FULL 1
#define BATTERY_CHECK_LOOP	(HZ * 30) //add for zte project 20130624
static int battery_cap_low = 0; //add for zte project 20130624
...
#define BATT_CHRG_FULL_DES		1980000
#endif
#ifdef CONFIG_PROJECT_V975
#define BATT_CHRG_FULL_DES		2280000
#endif
#ifdef CONFIG_PROJECT_P940T01
#define BATT_CHRG_FULL_DES		3200000
#endif
...
#define BATTERY_VOLT_MIN_THRESHOLD	3450000
...
#define LOW_VOLT_DEBOUNCE_TIME		4000
...
static uint16_t cell_char_tbl[] = {
	...
	0xA930, 0xB770, 0xB930, 0xBB00, 0xBBA0, 0xBBF0, 0xBC40, 0xBD00,
	0xBDA0, 0xBE80, 0xBF40, 0xC190, 0xC400, 0xC6A0, 0xC960, 0xCF50,
	/* Data to be written from 0x90h */
	0x0200, 0x1600, 0x0C30, 0x1400, 0x3600, 0x3400, 0x1F40, 0x2810,
	0x1AE0, 0x1EC0, 0x0BF0, 0x0BF0, 0x0BD0, 0x0BD0, 0x0850, 0x0850,
...
static uint16_t cell_char_tbl[] = {
	/* Data to be written from 0x80h */
	0xA5B0, 0xB710, 0xB930, 0xBB50, 0xBC10, 0xBD00, 0xBD90, 0xBE70,
	0xBF80, 0xC090, 0xC3B0, 0xC570, 0xC8B0, 0xCBD0, 0xD070, 0xD640,
	/* Data to be written from 0x90h */
	0x0110, 0x0E30, 0x0E00, 0x1570, 0x1D50, 0x2660, 0x1950, 0x16D0,
	0x16F0, 0x08F0, 0x0B10, 0x0B00, 0x07B0, 0x07F0, 0x0700, 0x0700,
	/* Data to be written from 0xA0h */
...
static uint16_t cell_char_tbl[] = {
	/* Data to be written from 0x80h */
	0xA8B0, 0xB720, 0xB800, 0xB9B0, 0xBAF0, 0xBC70, 0xBD00, 0xBE40,
	0xBFC0, 0xC0F0, 0xC400, 0xC720, 0xCC20, 0xCE70, 0xD1F0, 0xD7A0,
	/* Data to be written from 0x90h */
	0x0200, 0x1BB0, 0x0B00, 0x0C00, 0x1400, 0x1C60, 0x1700, 0x1230,
	0x12F0, 0x0990, 0x09D0, 0x07F0, 0x08F0, 0x06F0, 0x07A0, 0x07A0,
	/* Data to be written from 0xA0h */
	0x0180, 0x0180, 0x0180, 0x0180, 0x0180, 0x0180, 0x0180, 0x0180,
	0x0180, 0x0180, 0x0180, 0x0180, 0x0180, 0x0180, 0x0180, 0x0180,
};
...
struct max17042_chip {
    ...
    struct wake_lock wake_lock; //add for zte project 20130624
	...
	struct delayed_work evt_worker; //revised for zte project 20130624
...
static irqreturn_t max17042_thread_handler(int id, void *dev)
{
	...
	schedule_delayed_work(&chip->evt_worker,0); //revised for zte project 20130624
	pm_runtime_put_sync(&chip->client->dev);
	return IRQ_HANDLED;
}
...
#ifdef CONFIG_ZTE_BATTERY_LEVEL
static inline int max17042_volt_low(struct max17042_chip *chip)
{
	int volt_ocv, ret;
    int i,j,min_vol;
    int iTemp[6] = {0}; 
    ...
    {        
        ...
        if (ret < 0)
		    goto read_error;
	    iTemp[i] = (ret >> 3) * MAX17042_VOLT_CONV_FCTR;
        udelay(LOW_VOLT_DEBOUNCE_TIME);
    }
	...
	if (iTemp[1] <= BATTERY_VOLT_MIN_THRESHOLD)
		ret = 1;
	else
		ret = 0;
...
int max17042_battery_level_convert(int level)
{
	int cap_real = level;
	int cap_disp = 0;
...
#ifdef CONFIG_ZTE_CHARGE_FULL
static int battery_full_status = 0;
static int battery_keep_count = 0;
#endif
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (chip->pdata->is_volt_shutdown) {
			ret = max17042_volt_low(chip);
			if (ret < 0)
				goto ps_prop_read_err;
			if (ret && !(chip->disable_shdwn_methods &
		...
		if (chip->pdata->is_lowbatt_shutdown &&
			...
			/* to ensure it's not a temporary low-batt state
			 * caused by a burst pulse.
			 */
			ret = max17042_volt_low(chip);
			if (ret < 0)
				goto ps_prop_read_err;
        ...
        #ifdef CONFIG_ZTE_CHARGE_FULL
        if((POWER_SUPPLY_STATUS_CHARGING == chip->status) || (POWER_SUPPLY_STATUS_FULL == chip->status))
        {
            battery_keep_count = 0;
            if(val->intval > 95)
            {
                if(100 == val->intval)
                    battery_full_status = 1;

                if(1 == battery_full_status)
                {
                    val->intval = 100;
                }
            }
        ...
        else
        {
            ...
            {  
                ...
                {
                    battery_keep_count ++;
                    if(battery_keep_count >= 3)
                    {
                        battery_keep_count = 0;
                        battery_full_status = 0;
                    }
                }
                else if(val->intval >= 98)
                {
                    val->intval = 100;
                    battery_keep_count ++;
                    if(battery_keep_count >= 5)
                    {
                        val->intval = 99;
                        battery_keep_count = 0;
                        battery_full_status = 0;
        ...
        #ifdef CONFIG_ZTE_BATTERY_LEVEL
        val->intval = max17042_battery_level_convert(val->intval);
...
static void init_fg_config_data(void)
{
	...
	fg_conf_data->learn_cfg = 0x2606;
	fg_conf_data->filter_cfg = 0x87A6;
	fg_conf_data->relax_cfg = 0x506B;
	memcpy(&fg_conf_data->cell_char_tbl, cell_char_tbl,
					sizeof(cell_char_tbl));
	fg_conf_data->rcomp0 = 0x0058;
	fg_conf_data->tempCo = 0x121F;
	fg_conf_data->vempty = 0xA7DA;
    fg_conf_data->qrtbl00 = 0x2480;
    fg_conf_data->qrtbl10 = 0x1100;
    fg_conf_data->qrtbl20 = 0x0782;
    fg_conf_data->qrtbl30 = 0x0602;
    fg_conf_data->ichgt_term = 0x03C0;
    fg_conf_data->full_soc_thr = 0x5F00;
	fg_conf_data->full_cap = 3960;
	fg_conf_data->design_cap = 3960;
	fg_conf_data->full_capnom = 3960;
...
static void init_fg_config_data(void)
{
	...
	memcpy(&fg_conf_data->cell_char_tbl, cell_char_tbl,
					sizeof(cell_char_tbl));
	fg_conf_data->rcomp0 = 0x004B;
	fg_conf_data->tempCo = 0x1E28;
	fg_conf_data->vempty = 0xA7DA;
    fg_conf_data->qrtbl00 = 0x3000;
    fg_conf_data->qrtbl10 = 0x1500;
    fg_conf_data->qrtbl20 = 0x0800;
    fg_conf_data->qrtbl30 = 0x0600;
    fg_conf_data->ichgt_term = 0x03C0;
    fg_conf_data->full_soc_thr = 0x5F00;
	fg_conf_data->full_cap = 4560;
	fg_conf_data->design_cap = 4560;
	fg_conf_data->full_capnom = 4560;
	fg_conf_data->rsense = 1;
}
#endif
...
static void init_fg_config_data(void)
{
	...
	memcpy(&fg_conf_data->cell_char_tbl, cell_char_tbl,
					sizeof(cell_char_tbl));
	fg_conf_data->rcomp0 = 0x0050;
	fg_conf_data->tempCo = 0x1B32;
	fg_conf_data->vempty = 0xA7DA;
    fg_conf_data->qrtbl00 = 0x4300;
    fg_conf_data->qrtbl10 = 0x1F00;
    fg_conf_data->qrtbl20 = 0x0D80;
    fg_conf_data->qrtbl30 = 0x0A80;
    fg_conf_data->ichgt_term = 0x03C0;
    fg_conf_data->full_soc_thr = 0x5F00;
	fg_conf_data->full_cap = 6400;
	fg_conf_data->design_cap = 6400;
	fg_conf_data->full_capnom = 6400;
	fg_conf_data->rsense = 1;
}
#endif
...
static void dump_fg_conf_data(struct max17042_chip *chip)
{
	...
	for (i = 0; i < CELL_CHAR_TBL_SAMPLES; i++)
		dev_info(&chip->client->dev, "%x, ",fg_conf_data->cell_char_tbl[i]);
...
static void set_soc_intr_thresholds_s0(struct max17042_chip *chip, int offset)
{
    ...
    if(0 == soc)    //add for zte project 20130624
        battery_cap_low = 1;
    else
        battery_cap_low = 0;
...
static void max17042_evt_worker(struct work_struct *work)
{
    ...
    if(1 == battery_cap_low)    //add for zte project 20130624
    {
        schedule_delayed_work(&chip->evt_worker, BATTERY_CHECK_LOOP);
...
static void max17042_external_power_changed(struct power_supply *psy)
{
	struct max17042_chip *chip = container_of(psy,
			struct max17042_chip, battery);
	schedule_delayed_work(&chip->evt_worker,0);
}
			...
			const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&chip->evt_worker, max17042_evt_worker);  //revised for zte project 20130624
	wake_lock_init(&chip->wake_lock, WAKE_LOCK_SUSPEND, "max17047_wake_lock");//add for zte project 20130624
