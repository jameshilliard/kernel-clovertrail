...
static void dlp_ctrl_complete_tx(struct hsi_msg *msg)
{
	...
	complete(&ch_ctx->tx.cmd_txfer_done);
...
static void dlp_ctrl_complete_tx_async(struct hsi_msg *msg)
{
	...
	if (DLP_CMD_ACK == dlp_cmd->params.id){
		struct dlp_channel *ch_ctx = dlp_cmd->channel;
		
		/* notify rx thread */
		complete(&ch_ctx->rx.data_xfer_notify);
	}
	/*-- cj modify for marvell modem 20130310 --*/
}
...
static void dlp_ctrl_complete_rx(struct hsi_msg *msg)
{
	...
	if (dlp_drv.tty_sync_mode){
		if (DLP_CMD_OPEN_CONN == params.id){
	...
	/*++ cj modify for marvell modem bootrom bug 20130318 ++*/
	if (DLP_CMD_CONN_READY == params.id){
		//pr_debug(DRVNAME ": RX READY in ch0 , modify to ch1\n");
		if (0 == params.channel)
			params.channel = 1;
	}
	/*-- cj modify for marvell modem bootrom bug 20130318 --*/
	...
	case DLP_CMD_OPEN_CONN:
		...
		if (1){
			...
			memcpy(&hsi_ch->open_conn_rx, &params, sizeof(params));
			response = DLP_CMD_ACK;
			//pr_debug(DRVNAME ": HSI CH%d OPEN_CONN received\n",	params.channel);
	...
	case DLP_CMD_CONN_CLOSED:
		...
		complete(&ch_ctx->tx.data_xfer_close);
	/*-- cj modify for marvell modem 20130319 --*/
		break;

	/*++ cj modify for marvell modem 20130309 ++*/
	case DLP_CMD_CONN_READY:
		//pr_debug(DRVNAME ": ch%d conn_ready received\n", elp_channel);
				
		complete(&ch_ctx->tx.data_xfer_ready);
		break;
		
	case DLP_CMD_ECHO:
		pr_debug(DRVNAME ": ch%d cmd %d received\n",
				elp_channel, params.id);		
	/*-- cj modify for marvell modem 20130309 --*/
	...
	default:
		...
		/* Command done, notify the sender */
		complete(&ch_ctx->rx.cmd_rxfer_done);
		break;
	}
				...
				unsigned char param2, unsigned char param3)
{
	...
	ret = wait_for_completion_timeout(&ch_ctx->tx.cmd_txfer_done,
	...
	if (dlp_cmd->status) {
		pr_err(DRVNAME ": Failed to send cmd:0x%X ch:%d status %d %x %x\n",
				dlp_cmd->params.id, dlp_cmd->params.channel, dlp_cmd->status,
				dlp_cmd->channel->ch_id, dlp_cmd->channel->ch_data);
   ...
   /* 1. Set the intermidiate channel state */
	...
	ret = wait_for_completion_timeout(&ch_ctx->rx.cmd_rxfer_done,
	...
	ret = 0;
/*++ cj modify for marvell modem 20130317 ++*/
    if (expected_resp.id != response_id){
	...
	/*
	pr_err(DRVNAME": cmd 0x%X response 0x%X (expected 0x%X)",
			id,
			(unsigned int)(*(u32 *)&ctrl_ctx->response.params),
			(unsigned int)(*(u32 *)(&expected_resp)));
	*/
...
/*++ cj modify for marvell modem 20130309 ++*/
	...
	init_completion(&ch_ctx->handshake_over);
...
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_open_channel_with_msg(struct dlp_channel *ch_ctx, struct hsi_msg *pdu)
{
	int ret = 0;
	unsigned int txlen;

	txlen = pdu->actual_len;
	
	if (dlp_drv.obm_mode)
		txlen = txlen >> 2;
		
	unsigned char param1 = PARAM1(txlen);
	unsigned char param2 = PARAM2(txlen);
	unsigned char param3 = PARAM3(txlen);
	...
	if (ret == 0) {
		...
		/* Reset old OPEN_CONN params */
		dlp_drv.channels_hsi[ch_ctx->hsi_channel].open_conn_tx = 0;
	}
...
/*++ cj modify for marvell modem 20130310 ++*/
/*
* @brief Close the specified channel without ack
*	- Send the CLOSE_CONN command to the modem
*	- Return the operation status
*
* @param ch_ctx : The channel context to consider
...
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_close_channel_noack(struct dlp_channel *ch_ctx)
{
	int state, ret = 0;
	...
	/* Check if the channel was correctly opened */
		/* Send the command */
	ret = dlp_ctrl_cmd_send(ch_ctx,
			DLP_CMD_CONN_CLOSED, DLP_CMD_NONE,
			DLP_CH_STATE_CLOSING, DLP_CH_STATE_CLOSED,
			0, 0, 0);
...
/*++ cj modify for marvell modem 20130310 ++*/
/*
* @brief Send the ECHO command
*
* @param ch_ctx : The channel context to consider
*
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_send_echo(struct dlp_channel *ch_ctx)
{
	int ret;
	unsigned char param1, param2, param3;
...
/*
* @brief Send the ALLOCATE CHANNEL command
*
* @param ch_ctx : The channel context to consider
*
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_send_allocate_ch(struct dlp_channel *ch_ctx)
{
	int ret;
	unsigned char param1, param2, param3;
...
/*
* @brief Send the INFO_REQ command
*
* @param ch_ctx : The channel context to consider
*
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_send_inforeq(struct dlp_channel *ch_ctx)
{
	int ret;
	unsigned char param1, param2, param3;
...
/*
* @brief Send the CONFIGRUE command
*
* @param ch_ctx : The channel context to consider
*
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_send_configure(struct dlp_channel *ch_ctx)
{
	int ret;
	unsigned char param1, param2, param3;
...
/*
* @brief Send the ready command
*
* @param ch_ctx : The channel context to consider
*
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_send_ready(struct dlp_channel *ch_ctx)
{
	int ret;
	unsigned char param1, param2, param3;
	...
	/* Send the NOP command */
	ret = dlp_ctrl_cmd_send(ch_ctx,
			DLP_CMD_CONN_READY, DLP_CMD_NONE,
			DLP_CH_STATE_OPENED, DLP_CH_STATE_OPENED,
			param1, param2, param3);
...
/*
* @brief Send the HANDSHAKE command
*
* @param ch_ctx : The channel context to consider
*
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_send_handshake(struct dlp_channel *ch_ctx, int action)
{
	int ret;
	unsigned char param1, param2, param3;
	...
	/* Send the NOP command */
	ret = dlp_ctrl_cmd_send(ch_ctx,
			DLP_CMD_HANDSHAKE, DLP_CMD_HANDSHAKE,
			DLP_CH_STATE_NONE, DLP_CH_STATE_NONE,
			param1, param2, param3);
