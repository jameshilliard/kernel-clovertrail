...
#define HSIDL_ENFORCED_PADDING 4   // 16bytes = 4dwords padding
#define PAYLOAD2PAD(x) (((u32)(x) + ((HSIDL_ENFORCED_PADDING) - 1)) & ~((HSIDL_ENFORCED_PADDING) - 1))
/*-- ZTE:cj modify for marvell modem 20130310 --*/
...
/*++ ZTE:cj workaround for marvell modem recv 0x0000000 before ready cmd send 20130322 ++*/		
#ifdef CONFIG_MARVELL_MODEM_HSI
		is_first_data = pio_ctx->offset == 0;
#endif
		while ((ioread32(ARASAN_REG(HSI_STATUS)) &
			...
			if (likely(pio_ctx->blk->length > 0)) {
				...
				if(is_first_data){
					do{
						data = ioread32(ARASAN_CHN_REG(RX_DATA, i));
						if (data)
							break;
						printk("***hsi_timeout pre-check ch%d rx 0x%08x\n", i, data);
					}while((ioread32(ARASAN_REG(HSI_STATUS)) &	ARASAN_RX_NOT_EMPTY(i)));
					is_first_data = false;
					if (!data){
						//printk("***hsi_timeout re-enable rx int of ch%d \n", i);
						irq_cfg = ARASAN_IRQ_RX_THRESHOLD(i);
				...
				*buf = data;
				//printk("***hsi_timeout ch%d rx 0x%08x\n", i, *buf);
				msg->actual_len += HSI_FRAMES_TO_BYTES(1);
				pio_ctx->offset += 1;
			}
#endif
...
/*++ ZTE:cj modify for marvell modem 20130514 ++*/
#ifdef CONFIG_MARVELL_MODEM_HSI
static bool hsi_dma_enable;
void hsi_set_dma_enable(bool val){
	printk(KERN_DEBUG "%s set %s", __func__, val ? "enable" : "disable");
	hsi_dma_enable = val;
}
#endif
/*-- ZTE:cj modify for marvell modem 20130514 --*/
...
/*++ ZTE:cj modify for marvell modem 20130516 ++*/
#ifndef CONFIG_MARVELL_MODEM_HSI
	rx_timeout = rounddown_pow_of_two(200000/intel_hsi->ip_freq);
#else
	rx_timeout = rounddown_pow_of_two(200/intel_hsi->ip_freq);
#endif
/*-- ZTE:cj modify for marvell modem 20130516 --*/
...
#ifdef CONFIG_MARVELL_MODEM_HSI
		if (!hsi_dma_enable){
			intel_hsi->tx_dma_ch[i]   = -1;
			intel_hsi->rx_dma_ch[i]   = -1;
			continue;
		}
#endif
/*-- ZTE:cj modify for marvell modem 20130514 --*/
...
/*++ ZTE:cj modify for marvell modem 20130322 ++*/
	if (msg->status == HSI_STATUS_ERROR){
		printk("hsi_pio_xfer_complete line%d, retry error pdu\n", __LINE__);
		msg->status = HSI_STATUS_PROCEEDING;
	}
/*-- ZTE:cj modify for marvell modem 20130322 --*/
#endif
...
/*++ cj modify for marvell modem 20130321 ++*/
			if (0 != msg->channel){
				// 16 bytes padding for marvell modem
				//printk("***hsi_pio_xfer_complete ch%d sz%d\n", msg->channel, sz);
				sz = PAYLOAD2PAD(sz);
				//printk("***hsi_pio_xfer_complete ch%d padding sz%d\n", msg->channel, sz);
			}
/*-- cj modify for marvell modem 200130321 --*/
#endif
			for (; sz > 0; sz--) {
				if (is_tx)
					iowrite32(*buf, fifo);
...
/*++ ZTE:cj modify for marvell modem 20130315 ++*/
#ifndef CONFIG_MARVELL_MODEM_HSI
	pm_runtime_allow(dev);
#else
//2 temporary disable rt pm
