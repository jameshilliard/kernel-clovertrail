...
static ssize_t bcm_proc_write(struct file *filp,
		const char *buff, size_t len, loff_t *off)
{
	char messages[128];
       ...
       int count=0;
       int i=0;
       unsigned char temp_buffer[5]={0x10, 0x20, 0x00, 0x01, 0x01}; // B0 NFC chip
       unsigned char temp_rd_buffer[7]={0x0};
       /* the response should be 0x10,0x40,0x0,0x3,0x0,0x10,0x1 */
	if (copy_from_user(messages, buff, len))
       ...
       mdelay(20);
       count=sizeof(temp_buffer);
       for(i=0;i<5;i++)
       {
           printk("bcm i2c_master_send\r\n");
           ret = i2c_master_send(p_client, temp_buffer, count);
    	    if (ret != count) {
               printk("failed to write %d \r\n", ret);
            ret = -EIO;
            }
            mdelay(1000);
            ret=i2c_master_recv(p_client, temp_rd_buffer, 7);
            if(ret==7)
            {
            printk("bcm i2c_master_recv 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x \r\n",
                temp_rd_buffer[0],temp_rd_buffer[1],temp_rd_buffer[2],temp_rd_buffer[3],
                temp_rd_buffer[4],temp_rd_buffer[5],temp_rd_buffer[6]);
        }
        }
...
static struct file_operations bcm_proc_ops = {
    .owner   = THIS_MODULE,
    .write =bcm_proc_write,
};

static int create_bcm_proc_file(void)
{
    struct proc_dir_entry *entry;

    entry = create_proc_entry("driver/bcm_test", 0, NULL);
    if (entry)
        entry->proc_fops = &bcm_proc_ops;
    return 0;
}
			   ...
			   const struct i2c_device_id *id)
{
	...
	ret = gpio_request(platform_data->irq_gpio, "NFC-i2c-req");
	if (ret)
        return -ENODEV;
	ret = gpio_request(platform_data->en_gpio, "NFC-enable");
	if (ret)
		goto err_en;
	ret = gpio_request(platform_data->wake_gpio, "NFC-wake");
	if (ret)
        goto err_firm;
