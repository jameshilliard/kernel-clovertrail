...
static struct sensors_item_info sensors_info_list[] = {
	SENSORS_INFO(0x0F, ID_EXIST, 0x0F, 0x05, 5, SENSORS_TYPE_ACCEL, SENSORS_VENDOR_KIONIX),
	SENSORS_INFO(0x18, ID_EXIST, 0x0F, 0x33, 5, SENSORS_TYPE_ACCEL, SENSORS_VENDOR_ST),
	SENSORS_INFO(0x19, ID_EXIST, 0x0F, 0x33, 5, SENSORS_TYPE_ACCEL, SENSORS_VENDOR_ST),
	SENSORS_INFO(0x0C, ID_EXIST, 0x00, 0x48, 5, SENSORS_TYPE_COMPASS, SENSORS_VENDOR_AKM),
	SENSORS_INFO(0x69, ID_NOEXIST, 0x00, 0x00, 5, SENSORS_TYPE_GYRO, SENSORS_VENDOR_ST),
	SENSORS_INFO(0x6B, ID_EXIST, 0x0F, 0xD4, 5, SENSORS_TYPE_GYRO, SENSORS_VENDOR_ST),
	SENSORS_INFO(0x39, ID_EXIST, 0x92, 0x29, 5, SENSORS_TYPE_LIGHT, SENSORS_VENDOR_TAOS),
};
 /* ZTE:modify by liyongzhuang for accel kxtf9 --*/
...
static int sensor_check(struct sensors_item_info * sensor_item)
{
        ...
        if((ID_EXIST == sensor_item->id_exist) && (msgbuf != sensor_item->id_val)){
         ...
         printk("%s:[i2c-dev 0x%02x]device id  match success (0x%02x<->0x%02x)\n", __func__, \
                sensor_item->addr, msgbuf, sensor_item->id_val);
        return 0;
}
