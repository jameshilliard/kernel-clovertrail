...
#define TMD2771X_STATUS_INTMASK     0x30
...
#define TMD2771X_ISINT(x) (!!((x) & TMD2771X_STATUS_INTMASK))
...
struct delayed_work intlost_work;
...
static void tmd2771x_enabledisable(void){
	...
	//cancel_delayed_work_sync(&intlost_work);
	...
	if(g_prox_config.enabled ||g_als_config.enabled){
		...
		if(g_prox_config.enabled){
			//schedule_delayed_work(&intlost_work, msecs_to_jiffies(INT_LOST_POLL_MS));
		}
...
static void tmd2771x_intlost_func(struct work_struct * work)
{
	int ret;
	...
	if(TMD2771X_ISINT(status)){
		/*clear the interrupt status*/
        	printk("%s: int status = 0x%02x \n", __func__, status);
		tmd2771x_sfcommand(TMD2771X_CMD_PROXALS_INTCLR);
		schedule_work(&datap->work);
	}
...
static int tmd2771x_probe(struct i2c_client *clientp, const struct i2c_device_id *dev_id)
{
	...
	INIT_DELAYED_WORK(&intlost_work, tmd2771x_intlost_func);
	...
	clientp->irq = gpio_to_irq(63);
	ret = request_threaded_irq(clientp->irq, NULL, tmd2771x_irq_handler, \
                    IRQF_TRIGGER_FALLING, "taos_irq", datap);
	if (ret != 0) {
