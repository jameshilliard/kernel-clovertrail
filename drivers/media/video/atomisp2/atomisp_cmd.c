...
void atomisp_set_term_en_count(struct atomisp_device *isp)
{
	...
	val |= 0x1C << TERM_EN_COUNT_1LANE_OFFSET; // 0xf  0X1C  //0X28  // modify by zte yaolingling for F01 , intel let me do.

	/* set TERM_EN_COUNT_4LANE to 0xf */
	val &= pwn_b0 ? ~TERM_EN_COUNT_4LANE_PWN_B0_MASK :
				~TERM_EN_COUNT_4LANE_MASK;
	if(!strncmp(isp->inputs[isp->input_curr].camera->name, "imx091", 6)) {
