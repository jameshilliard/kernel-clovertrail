 ...
 */
 /* del by  ZTE yaoling for f01/v10 ,it is no useless for them ,start */
 #if 0
...
#endif 
 /* del by  ZTE yaoling for f01/v10 ,it is no useless for them ,end  */
				  ...
				  struct v4l2_private_int_data *priv)
{
	...
	int r = 0;
       dev_info(&client->dev,"ov8830_g_priv_int_data\n");
       if (copy_to_user(priv->data,g_vcm ,min(priv->size, sizeof(g_vcm) )))
		r = -EFAULT;
	priv->size =sizeof(g_vcm);// size;
	dev_info(&client->dev,"ov8830_g_priv_int_data r=%d,priv->size =%d\n",r,(int)priv->size );
	return r;
}
			...
			int dig_gain, u16 *hts, u16 *vts)
{
	...
	if (dig_gain) {
               /*  del by ZTE yaolingling because it affects wb of otp start */
                #if 0
		...
		if (ret)
			return ret;
               #endif  
               /* del by ZTE yaolingling because it affects wb of otp end */
	}
...
/* OTP */
/* for F01 and V10 the golden module as bellow */
 int BG_Ratio_Typical = 0x116;
int RG_Ratio_Typical  = 0x102;
struct otp_struct {
	u16 module_integrator_id;
	u16 lens_id;
	u16 production_year; // add new
	u16 production_month; // add new
	u16 production_day;  // add new
	u16 rg_ratio;
	u16 bg_ratio;
	u16 light_rg;
	u16 light_bg;
...
int check_otp_wb(struct i2c_client *client,int index)
{
	...
	// select bank index
	bank = 0xc0| index ;
	dev_info(&client->dev,"check_otp_wb  bank=%x\n",bank);
	ov8830_write_reg(client,OV8830_8BIT,0x3d84, bank);
	
	// read otp into buffer
	ov8830_write_reg(client,OV8830_8BIT,0x3d81, 0x01);
	...
	//dev_info(&client->dev,"check_otp_wb   address=%x\n",address);
	//#endif
	 ov8830_read_reg(client,OV8830_8BIT,address,&flag);
	//#if OV8825_PRINT
	dev_info(&client->dev,"check_otp_wb  flag=%d\n",flag);
	//#endif
	flag = flag & 0xc0;
	// disable otp read
	//ov8830_write_reg(client,OV8830_8BIT,0x3d81, 0x00);

	// clear otp buffer
	//for (i=0;i<32;i++) {
	for (i=0;i<16;i++) {
		ov8830_write_reg(client,OV8830_8BIT,0x3d00 + i, 0x00);
	}

	if (flag == 0x00) {
		dev_info(&client->dev,"check_otp_wb  return 0: no otp data : empty\n");
		return 0;
	}
	//else if ((!(flag & 0x80)) && (flag & 0x7f)) {
	else if (flag & 0x80) {
		dev_info(&client->dev,"check_otp_wb  otp invalid\n");
		return 1;
	}
	else {
		dev_info(&client->dev,"check_otp_wb valid and success \n");
		return 2;
	}
}
...
int check_otp_lenc(struct i2c_client *client,int index)
{
	...
	bank = 0xc0 |(index * 4);
	 dev_info(&client->dev,"check_otp_lenc  bank=%x\n",bank);
	ov8830_write_reg(client,OV8830_8BIT,0x3d84, bank);
	...
	address = 0x3d00;
	 ov8830_read_reg(client,OV8830_8BIT,address,&flag);
	 dev_info(&client->dev,"check_otp_lenc  flag=%d\n",flag);
	flag = flag & 0xc0;
	...
	if (flag == 0x00) {
		dev_info(&client->dev,"check_otp_lenc  return 0: no otp data : empty \n ");
		return 0;
	}
	else if (flag & 0x80) {
		dev_info(&client->dev,"check_otp_lencs  otp invalid \n ");
		return 1;
	}
	else {
		dev_info(&client->dev,"check_otp_lenc  valid and success\n ");
		return 2;
	}
}
...
int check_otp_VCM(struct i2c_client *client,int index,int code)
{
	...
	bank = 0xc0 +16;
	dev_info(&client->dev,"check_otp_VCM  bank=%x\n",bank);
	ov8830_write_reg(client,OV8830_8BIT,0x3d84, bank);
	// read otp into buffer
	ov8830_write_reg(client,OV8830_8BIT,0x3d81, 0x01);
       mdelay(5);
	...
	if (flag == 0x00) {
		dev_info(&client->dev,"check_otp_vcm  return 0: no otp data : empty \n ");
		return 0;
	}
	else if (flag & 0x80) {
		dev_info(&client->dev,"check_otp_vcm invalid  )\n ");
		return 1;
	}
	else {
		dev_info(&client->dev,"check_otp_vcm  valid and success\n ");
		return 2;
	}
...
// return: 	0, 
int read_otp_wb(struct i2c_client *client,int index, struct otp_struct * otp_ptr)
{
	int i,bank;
	int address;
	u16 temp ;
	...
	//address = 0x3d05 + index*9;
	address = 0x3d00;
	ov8830_read_reg(client,OV8830_8BIT,address+1,&((*otp_ptr).module_integrator_id));
	dev_info(&client->dev,"read_otp_wb (*otp_ptr).module_integrator_id=%x \n ",(*otp_ptr).module_integrator_id);
	ov8830_read_reg(client,OV8830_8BIT,address + 2,&((*otp_ptr).lens_id));
	ov8830_read_reg(client,OV8830_8BIT,address + 3,&((*otp_ptr).production_year));
	ov8830_read_reg(client,OV8830_8BIT,address + 4,&((*otp_ptr).production_month));
	ov8830_read_reg(client,OV8830_8BIT,address + 5,&((*otp_ptr).production_day));
       ov8830_read_reg(client,OV8830_8BIT,address + 10,&temp);

	
	ov8830_read_reg(client,OV8830_8BIT,address + 6,&((*otp_ptr).rg_ratio));
	(*otp_ptr).rg_ratio =( (*otp_ptr).rg_ratio <<2) +((temp >>6)&0x03);
	ov8830_read_reg(client,OV8830_8BIT,address + 7,&((*otp_ptr).bg_ratio));
	(*otp_ptr).bg_ratio =( (*otp_ptr).bg_ratio <<2) +((temp >>4)&0x03);
	
	ov8830_read_reg(client,OV8830_8BIT,address + 8,&((*otp_ptr).light_rg));
	(*otp_ptr).light_rg =( (*otp_ptr).light_rg <<2) +((temp >>2)&0x03);
	ov8830_read_reg(client,OV8830_8BIT,address + 9,&((*otp_ptr).light_bg));
	(*otp_ptr).light_bg =( (*otp_ptr).light_bg <<2) +(temp &0x03);
	ov8830_read_reg(client,OV8830_8BIT,address + 11,&((*otp_ptr).user_data[0]));
	ov8830_read_reg(client,OV8830_8BIT,address + 12,&((*otp_ptr).user_data[1] ));
	ov8830_read_reg(client,OV8830_8BIT,address + 13,&((*otp_ptr).user_data[2]));
	ov8830_read_reg(client,OV8830_8BIT,address + 14,&((*otp_ptr).user_data[3] ));
	ov8830_read_reg(client,OV8830_8BIT,address + 15,&((*otp_ptr).user_data[4]));
	...
	for (i=0;i<16;i++) {
      ...
      dev_info(&client->dev,"exit read_otp_wb \n ");
	return 0;	
}
...
// otp_ptr: pointer of otp_struct
// return: 	0, 
int read_otp_lenc(struct i2c_client *client, int index, struct otp_struct * otp_ptr)
{
	int bank, i;
	int address;
	...
	address = 0x3d01;
	for(i=0;i<15;i++) {
		ov8830_read_reg(client, OV8830_8BIT,address, &((* otp_ptr).lenc[i]));
		address++;
	}
	...
	dev_info(&client->dev,"exit read_otp_lens \n ");
	return 0;	
}
// index: index of otp group. (1, 2, 3)
// code: 0 start code, 1 stop code
// return: 0
int read_otp_VCM(struct i2c_client *client,int index, int code, struct otp_struct * otp_ptr)
{
	int  i, bank;
	int address;
	u16 vcm, vcm_temp;
	...
	// read flag
	address = 0x3d00 + (index-1)*4 + code*2;
	 ov8830_read_reg(client,OV8830_8BIT,address,&vcm);
	ov8830_read_reg(client,OV8830_8BIT,(address+1),&vcm_temp);
	dev_info(&client->dev,"  read_otp_VCM  vcm_temp=%x\n ",vcm_temp);
	vcm_temp = vcm_temp << 2;
	vcm = (vcm & 0x03) + vcm_temp;
	dev_info(&client->dev,"  read_otp_VCM  vcm=%x\n ",vcm);
	if(code==1) {
	(* otp_ptr).VCM_end = vcm;
	}
	else {
...
// return 0;
int update_awb_gain(struct i2c_client *client,int R_gain, int G_gain, int B_gain)
{
	if (R_gain>0x400) {
		//#if OV8825_PRINT
		dev_info(&client->dev,"  (R_gain>0x400) \n ");
		//#endif
		ov8830_write_reg(client,OV8830_8BIT,0x3400, R_gain>>8);
		ov8830_write_reg(client,OV8830_8BIT,0x3401, R_gain & 0x00ff);
	}

	if (G_gain>0x400) {
		//#if OV8825_PRINT
		dev_info(&client->dev,"  (G_gain>0x400) \n ");
		//#endif
		ov8830_write_reg(client,OV8830_8BIT,0x3402, G_gain>>8);
		ov8830_write_reg(client,OV8830_8BIT,0x3403, G_gain & 0x00ff);
	}

	if (B_gain>0x400) {
		//#if OV8825_PRINT
		dev_info(&client->dev,"  (B_gain>0x400) \n  ");
		//#endif
		ov8830_write_reg(client,OV8830_8BIT,0x3404, B_gain>>8);
		ov8830_write_reg(client,OV8830_8BIT,0x3405, B_gain & 0x00ff);
	}
	return 0;
}
// otp_ptr: pointer of otp_struct
int update_lenc(struct i2c_client *client,struct otp_struct * otp_ptr)
{
	int i, temp;
	u16 reg;

	ov8830_read_reg(client,OV8830_8BIT,0x5000,&reg);
	temp = 0x80 |reg;
	ov8830_write_reg(client,OV8830_8BIT,0x5000, temp);

	for(i=0;i<62;i++) {
		ov8830_write_reg(client,OV8830_8BIT,0x5800 + i, (*otp_ptr).lenc[i]);
	}
...
int update_otp_wb(struct v4l2_subdev *sd)
{
	...
	read_otp_wb(client,otp_index, &current_otp);
...
int update_otp_lenc(struct v4l2_subdev *sd)
{
	...
	if (i>3) {
		// no valid wb OTP data
		dev_info(&client->dev,"no valid lens OTP data\n ");
		return 0;
	}

	read_otp_lenc(client,otp_index, &current_otp);

	update_lenc(client,&current_otp);

	// success
	return 0;

}
static int get_VCM_start(struct v4l2_subdev *sd)
{
	struct otp_struct current_otp;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int i;
	...
	for(i=1;i<=3;i++){
		...
		if (i>3) {
		// no valid wb OTP data
		dev_info(&client->dev,"no valid vcm OTP data\n ");
		return 1;
	}
	read_otp_VCM(client, otp_index, 0, &current_otp);
	g_infinity = current_otp.VCM_start;
	g_vcm[0]= current_otp.VCM_start;
	v4l2_err(client, "get_VCM_start otpvcmstart=%d\n",current_otp.VCM_start);
	return current_otp.VCM_start ;
	
}
static int get_VCM_end(struct v4l2_subdev *sd)
{
	struct otp_struct current_otp;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int i;
	...
	read_otp_VCM(client, otp_index, 1, &current_otp);
	g_vcm[1] = current_otp.VCM_end; 
       v4l2_err(client, "get_otpVCM_end vcmend=%d\n",current_otp.VCM_end);
	return current_otp.VCM_end ;
	
}
// 1 use cp data from reg3d0a
// 2 use module data from reg 3d0a 
// 0 data error
int updata_blc_ratio(struct v4l2_subdev *sd)
{
	u16  k;
	u16 temp;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	ov8830_write_reg(client,OV8830_8BIT,0x3d84, 0xdf);
	ov8830_write_reg(client,OV8830_8BIT,0x3d81, 0x01);

	mdelay(5);
	ov8830_read_reg(client,OV8830_8BIT,0x3d0b,&k);
	v4l2_err(client, "updata_blc_ratio  0x3d0b  k=%x\n",k); //
		if(k != 0){
			if(k>=0x15 && k<=0x40){
				// auto load mode
				ov8830_read_reg(client,OV8830_8BIT,0x4008,&temp);
				temp&=0xfb;
				ov8830_write_reg(client,OV8830_8BIT,0x4008, temp);

				temp = ov8830_read_reg(client,OV8830_8BIT,0x4000,&temp);
				temp&=0xf7;
				ov8830_write_reg(client,OV8830_8BIT,0x4000, temp);
				 v4l2_err(client, "updata_blc_ratio  return 2\n");
				return 2;
				}
			}
		ov8830_read_reg(client,OV8830_8BIT,0x3d0a,&k);
		v4l2_err(client, "updata_blc_ratio 0x3d0a  k=%x\n",k);
		if(k>=0x10 && k<= 0x40){
			// manual load mode
			ov8830_write_reg(client,OV8830_8BIT,0x4006, k);
			ov8830_read_reg(client,OV8830_8BIT,0x4008,&temp);
			temp &=0xfb;
			ov8830_write_reg(client,OV8830_8BIT,0x4008, temp);

			ov8830_read_reg(client,OV8830_8BIT,0x4000,&temp);
			temp |=0x08;
			ov8830_write_reg(client,OV8830_8BIT,0x4000, temp);
			 v4l2_err(client, "updata_blc_ratio  return 1\n");
			return 1;

			}
		else{
			// set to default
			ov8830_write_reg(client,OV8830_8BIT,0x4006, 0x20);
			ov8830_read_reg(client,OV8830_8BIT,0x4008,&temp);
			temp &=0xfb;
			ov8830_write_reg(client,OV8830_8BIT,0x4008, temp);
			...
			temp |=0x08;
			ov8830_write_reg(client,OV8830_8BIT,0x4000, temp);
			 v4l2_err(client, "updata_blc_ratio  return 0\n");
			return 0;
...
static int __ov8830_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on == 0) {
		...
		if(ret)
			return ret ;
		/* ZTE  del by yaoling for f01 and v10 do not use this driver start */
	        //ret  = drv201_power_down(sd);
	        //	if (ret == 0)
		//	ret = r; // del by yaoling
		/* ZTE  del by yaoling for f01 and v10 do not use this driver end */
		dev->power = 0;
	} else {
		ret = power_up(sd);
		if (ret)
			return ret;
		/* ZTE  del by yaoling for f01 and v10 do not use this driver */
//drv201_power_up(sd);  // del by yaoling
		...
		ret = ov8830_init_registers(sd);
                /* for get otp data */
		ov8830_write_reg(client, OV8830_8BIT, 0x4202, 0x0f);
		ov8830_write_reg(client, OV8830_8BIT, 0x0100, 0x01);		
		get_VCM_start(sd);
		get_VCM_end(sd);
		update_otp_wb(sd);
		update_otp_lenc(sd);
		ov8830_write_reg(client, OV8830_8BIT, 0x4202, 0x00);
		ov8830_write_reg(client, OV8830_8BIT, 0x0100, 0x00);
	}
...
static int nearest_resolution_index(struct v4l2_subdev *sd, int w, int h)
{
	...
	struct ov8830_device *dev = to_ov8830_sensor(sd);
	/* add  by yaoling for video switch 1080P to QVGA   green issue  0621 start  */
	if((w == 320 && h == 240) || (w == 336 && h == 256))
	{
		return 2;
	}
	/* add  by yaoling for video switch 1080P to QVGA   green issue  0621 end */
				 ...
				 struct v4l2_mbus_framefmt *fmt)
{
	int idx;
	struct ov8830_device *dev = to_ov8830_sensor(sd);
       struct i2c_client *client = v4l2_get_subdevdata(sd);  // add by yaoling for debug
	if (!fmt)
		return -EINVAL;
         v4l2_err(client, "ov8830_try_mbus_fmt dev->fmt_idx=%d,width=%d,height=%d\n",dev->fmt_idx,fmt->width, fmt->height);
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	dev->fmt_idx = get_resolution_index(sd, fmt->width, fmt->height);
	 v4l2_err(client, "ov8830_s_mbus_fmt dev->fmt_idx=%d, width =%d,height =%d\n",dev->fmt_idx,fmt->width,fmt->height);
	/* Sanity check */
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	if (!fmt)
		return -EINVAL;
        v4l2_err(client, "ov8830_g_mbus_fmt dev->fmt_idx=%d, width =%d,height =%d\n",dev->fmt_idx,fmt->width,fmt->height);
	mutex_lock(&dev->input_lock);
...
static int ov8830_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
       v4l2_err(client, "ov8830_s_stream enable=%d\n",enable);
	mutex_lock(&dev->input_lock);
			    ...
			    int irq, void *pdata)
{
	...
	mutex_lock(&dev->input_lock);
      v4l2_err(client, "	enter ov8830_s_config \n");
	...
	if (ret) {
       ...
       v4l2_err(client, "	exit   exit 8830_s_config\n");
	return 0;
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (ret) {
        ...
        v4l2_err(client, "after  v4l2_ctrl_handler_init\n");
	...
	v4l2_ctrl_handler_setup(&dev->ctrl_handler);
	  v4l2_err(client, "after  v4l2_ctrl_handler_setup\n");
	ret = media_entity_init(&dev->sd.entity, 1, &dev->pad, 0);
	if (ret) {
		ov8830_remove(client);
		return ret;
	}
         v4l2_err(client, "exit ov8830_probe\n");
	return 0;
