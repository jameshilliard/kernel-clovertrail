			   ...
			   int plug_status)
{
	int status = 0;
	unsigned int reg;
    pr_err("chaofubang:%s,plug_status:%d\n", __func__, plug_status);
	...
	if (plug_status) {
		...
		if (reg & MIC2_SDET) { /*button pressed */
			pr_err("%s:Button Pressed\n", __func__);
			status = SND_JACK_HEADSET | SND_JACK_BTN_0;
		} else {
			pr_err("%s:Button Released\n", __func__);
			   ...
			   int plug_status)
{
	...
	unsigned int reg;
        pr_err("%s,plug_status:%d\n", __func__, plug_status);

	if (plug_status) {
		pr_err("%s: Jack removed\n", __func__);
		...
		*/
		...
		udelay(100);
		reg = snd_soc_read(codec, CS42L73_IS1);
		pr_err("Mic detect = %x ISI =%x\n", micbias, reg);
		if (reg & MIC2_SDET) {
			status = SND_JACK_HEADPHONE;
			pr_err("Headphone detected\n");
		} else {
			status = SND_JACK_HEADSET;
			pr_err("Headset detected\n");
