...
unsigned int ext_pa_boost_switch;
unsigned int ext_pa_switch;
#endif
		...
		struct snd_pcm_hw_params *params)
{
	...
	switch (device) {
	case CTP_RHB_AUD_ASP_DEV:
#ifndef CONFIG_SND_SOC_CS42L73_VSP_MASTER
	case CTP_RHB_AUD_VSP_DEV:
#endif
	case CTP_RHB_AUD_VIRTUAL_ASP_DEV:
...
#ifdef CONFIG_SND_SOC_CS42L73_VSP_MASTER
    case CTP_RHB_AUD_VSP_DEV:
		clk_fmt.clk_id = CS42L73_CLKID_MCLK1;
		clk_fmt.freq = DEFAULT_MCLK;
		clk_fmt.dir = SND_SOC_CLOCK_IN;
...
#ifdef CTP_EXT_SPK_PA
static int ihf_ext_pa_init(void)
{
	/* struct snd_soc_codec *codec =  snd_kcontrol_chip(kcontrol); */
	int err;
	pr_err("%s\n", __func__);
	
	err = gpio_request(EXT_PA_BOOST_EN_PIN, "EXT_PA_BOOST_EN");
	if (err < 0) {
		pr_err("%s: gpio request failed\n", __func__);
		return err;
	}
	err = gpio_direction_output(EXT_PA_BOOST_EN_PIN, 0);
	if (err < 0) {
		pr_err("%s: gpio set direction failed\n", __func__);
		gpio_free(EXT_PA_BOOST_EN_PIN);
		return err;
	}
	
	err = gpio_request(EXT_PA_EN_PIN, "EXT_PA_BOOST_EN");
	if (err < 0) {
		pr_err("%s: gpio request failed\n", __func__);
		return err;
	}
	err = gpio_direction_output(EXT_PA_EN_PIN, 0);
	if (err < 0) {
		pr_err("%s: gpio set direction failed\n", __func__);
		gpio_free(EXT_PA_EN_PIN);
		return err;
	}
    ext_pa_boost_switch = 0;
    ext_pa_switch = 0;
	return 0;
}
#endif
