...
/* add by zte yaolingling for camera power start */
static int camera_v1p2_vcm_dvdd ;
static int camera_v2p8_vcm_vdd;
/* add by zte yaolingling for camera power end  */
...
static int ov8830_power_ctrl(struct v4l2_subdev *sd, int flag)
{
	...
	if (INTEL_MID_BOARD(2, PHONE, CLVTP, VB, PRO) ||
        ...
        /* add by ZTE yaolingling for camera power start */
        printk(KERN_ALERT "enter ov8830_power_ctrl\n");
	if (camera_v2p8_vcm_vdd < 0) {
		 printk(KERN_ALERT "enter ov8830_power_ctrl after if (camera_v2p8_vcm_vdd < 0)\n");
	...
	if (camera_v1p2_vcm_dvdd < 0) {
        ...
        /* add by ZTE yaolingling for camera power end */
	if (flag) {
                printk(KERN_ALERT "enter ov8830_power_ctrl poweron \n");
		if (!camera_vprog1_on) {
                       // camera_vprog1_on = 1;
		   ...
		   /* ZTE: add by yaoling for p940f01 */
		gpio_set_value(camera_power_down,1);
		 printk(KERN_ALERT "enter ov8830_power_ctrl before  gpio_set_value(camera_v2p8_vcm_vdd, 1);\n");
		gpio_set_value(camera_v2p8_vcm_vdd, 1);
		gpio_set_value(camera_v1p2_vcm_dvdd, 1);
	}
		else {
		if (camera_vprog1_on) {
                        printk(KERN_ALERT "enter ov8830_power_ctrl powerdown\n");
			if (intel_mid_identify_cpu() ==
			   INTEL_MID_CPU_CHIP_TANGIER)
		...
		/* ZTE: add by yaoling for p940f01 */
		 printk(KERN_ALERT "enter ov8830_power_ctrl before  gpio_set_value(camera_v2p8_vcm_vdd, 0);\n");
		gpio_set_value(camera_v2p8_vcm_vdd, 0);
		gpio_set_value(camera_v1p2_vcm_dvdd, 0);
		gpio_set_value(camera_power_down,0);
		printk(KERN_ALERT "ov8830 camera_v2p8_vcm_vdd free\n");
        ...
        printk(KERN_ALERT "exit ov8830_power_ctrl ret =%d\n",ret );
	return ret;
}
...
{
	int ret;
         printk(KERN_ALERT "enter ov8830_platform_init\n");
	if (intel_mid_identify_cpu() == INTEL_MID_CPU_CHIP_TANGIER)
		{
		dev_err(&client->dev, "ov8830_platform_init INTEL_MID_CPU_CHIP_TANGIER\n");	
		return 0;
		}
