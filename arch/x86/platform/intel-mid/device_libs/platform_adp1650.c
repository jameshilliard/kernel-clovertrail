/*
 * platform_adp1650.c: adp1650 platform data initilization file
 *
...
#include "platform_adp1650.h"

void *adp1650_platform_data_func(void *info)
{
	static struct adp1650_platform_data platform_data;

	platform_data.gpio_enable= get_gpio_by_name("GP_FLASH_RESET");
	platform_data.gpio_strobe = get_gpio_by_name("GP_FLASH_STROBE");
	platform_data.gpio_torch  = get_gpio_by_name("GP_FLASH_TORCH");

	if (platform_data.gpio_enable== -1) {
		pr_err("%s: Unable to find GP_FLASH_ENABLE\n", __func__);
		return NULL;
	}
