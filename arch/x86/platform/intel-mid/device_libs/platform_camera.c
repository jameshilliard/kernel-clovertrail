...
const struct intel_v4l2_subdev_id v4l2_ids[] = {
	...
	{"imx091", RAW_CAMERA, ATOMISP_CAMERA_PORT_PRIMARY},
	...
	{"lm3554", LED_FLASH, -1},
	{"lm3559", LED_FLASH, -1},
	/* ZTE: for camera module of p940f01 by yaolingling start */
	{"ov9740", SOC_CAMERA, ATOMISP_CAMERA_PORT_SECONDARY},
	{"sp1628", SOC_CAMERA, ATOMISP_CAMERA_PORT_SECONDARY},
	{"adp1650", LED_FLASH, -1},
	{"bu64291", CAMERA_MOTOR, -1},
	/* ZTE: for camera module of p940f01 by yaolingling end  */
	{"dw9714", CAMERA_MOTOR, -1},
	{},
};
...
int camera_sensor_gpio(int gpio, char *name, int dir, int value)
{
	...
	pr_err("%s: gpio_request  to set gpio ret=%d (pin %d) \n",
							__func__, ret,pin);
	if (ret) {
	...
	if (ret) {
		pr_err("%s: if (ret)  failed to set gpio ret=%d (pin %d) direction\n",
							__func__,	ret, pin);
		pr_err("%s: gpio_free  to set gpio ret=%d (pin %d) \n",
							__func__, ret,pin);
		gpio_free(pin);
	}
       pr_err("%s: exit camera_sensor_gpio ret=%d (pin %d) direction\n",
							__func__, ret,pin);
	return ret ? ret : pin;
}
...
{
	return  0;//gpio_direction_output(camera_af_power_gpio, flag);
}
